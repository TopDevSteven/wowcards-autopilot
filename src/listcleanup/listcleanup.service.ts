...
export class ListcleanupService {
  ...
  async listCleanUpPro(shop_group: ProChainGroup) {
    const res = await this.protractorDedupeService.list_cleanup(
      shop_group.shopname,
      shop_group.chain_id,
    );
  ...
  async listCleanUpSW(shop_group: SWChainGroup) {
    const res = await this.shopwareDedupeService.listLeanUp(
      shop_group.tenant_id,
      shop_group.chain_id,
      shop_group.shop_id,
    );
  ...
  async listCleanUpTek(shop_group: TekChainGroup) {
    const res = await this.tekmetricDedupeService.list_cleanup(
      shop_group.shop_id,
      shop_group.chain_id,
    );
  ...
  async mergeCleanUpData(allShops: ChainObejct) {
    const tekCustomers =
      allShops.tek.length != 0
        ? await Promise.all(
            allShops.tek.map((item) => this.listCleanUpTek(item)),
          )
        : [];
    ...
    const proCustomers =
      allShops.pro.length != 0
        ? await Promise.all(
            allShops.pro.map((item) => this.listCleanUpPro(item)),
          )
        : [];
  ...
  async addBadAddressFlag(allShops: ChainObejct) {
    ...
    const addedFlagCustomers = mergedCustomers.map((item) => {
      return {
        ...item,
        isBadAddress:
          item.address1.trim() === "" && item.address2.trim() === ""
            ? "Bad Address"
            : "",
        WCAID: "",
        MBDayMo: "",
  ...
  async addDupFlagBasedCI(allShops: ChainObejct) {
    ...
    const completedReport = addedFlagCustomers.map((customer) => {
      ...
      const key =
        chain_id != null
          ...
          : `${new_firstname}-${new_secondname}-${address1}-${address2}-${wow_shop_id}-1-${software}`;
      const isDuplicate =
        (counts.get(key) || 0) > 1 && mailables.get(key) !== authorizied_date
          ? "Duplicate"
          : "";
    ...
    const okBadyCustomers = completedReport.filter(
      (item) =>
        item.isBadAddress != "Bad Address" &&
        item.isDuplicate != "Duplicate" &&
        item.name_code != "Bad Name",
    );

    const cleanedreport = okBadyCustomers.sort((a, b) => {
      const dateComparison =
        a.authorizied_date.getTime() - b.authorizied_date.getTime();
    ...
    return cleanedreport.map((item) => {
      ...
      return {
        ...item,
        TBDayMo:
          item.b_month === "" || item.b_month === null
            ? ((count % 12) + 1).toString()
            : "",
      };
    });
