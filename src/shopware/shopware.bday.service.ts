            ...
            fs.createReadStream(filePath)
                ...
                .on('data',(data: {'Software': string, 'Shop Id': string; 'Customer ID': string; 'OK YEAR': string, 'OK MONTH': string }) => {
                    if (data['Software'].trim() === 'SW') {
                        const tenant_id: string | undefined = this.getKeyByValue(SWID, data['Shop Id']);
                      results.push({ storeId: data['Shop Id'], tanentID: tenant_id, customerId: data['Customer ID'], Year: data['OK YEAR'],  Month: data['OK MONTH'], Day: ""});
                    }
                  })
                .on('end', () => {
                    resolve(results);
