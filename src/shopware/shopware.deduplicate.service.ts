...
export class ShopWareDeduplicate {
  ...
  async listLeanUp(tenant_id: number, chainid: number | null, shop_id: number) {
    const rawCustomerData = await this.shopwareService.getSWReport(
      tenant_id,
      shop_id,
    );
    const nonNullCustomerData = rawCustomerData.map((customer) => {
      ...
      customer.first_name != null
        ? (nonNullCustomer.firstname = customer.first_name
            .replace(/\s+/g, " ")
            .trim())
        : "";
      customer.last_name != null
        ? (nonNullCustomer.lastname = customer.last_name
            .replace(/\s+/g, " ")
            .trim())
        : "";
      customer.address != null
        ? (nonNullCustomer.address = customer.address
            .replace(/\s+/g, " ")
            .trim())
        : "";
      customer.city != null
        ? (nonNullCustomer.city = customer.city.replace(/\s+/g, " ").trim())
        : "";
      customer.state != null
        ? (nonNullCustomer.state = customer.state.replace(/\s+/g, " ").trim())
        : "";
      customer.zip != null
        ? (nonNullCustomer.zip = customer.zip.replace(/\s+/g, " ").trim())
        : "";
      (nonNullCustomer.id = customer.id),
        customer.b_year != null
          ? (nonNullCustomer.b_year = customer.b_year)
          : "",
        customer.b_month != null
          ? (nonNullCustomer.b_month = customer.b_month)
          : "",
