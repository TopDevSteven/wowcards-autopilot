...
export class TekBdayService {
  ...
  async mergedTekDay() {
    const res1 = await this.parseCsvFile(
      "./Bdayfiles/Tekmertic__81,383_BDAY_Year_Month_Day.csv",
    );
    const res2 = await this.parseMergedCsvfile(
      "./Bdayfiles/wow_June_23_-_merged_Final.csv",
    );
  ...
  async writeBdayToDB(bday: TekBdayObject[]) {
    ...
    await this.db.query(
      ...
      [tekBday.ids, tekBday.storeids, tekBday.year, tekBday.month, tekBday.day],
    );
    ...
    await this.db.query(`DROP TABLE temp_table_tek;`);
