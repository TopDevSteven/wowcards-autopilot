...
const activeShopList = [
  398, 1159, 1552, 3028, 3586, 3229, 3472, 293, 1216, 4494, 888, 309, 2442,
  3543, 3547, 3758, 3540, 3539, 3761, 1873, 3542, 3541, 3759, 331, 1692, 2305,
  3520, 1398, 3351, 3385, 4120,
];
  ...
  async list_cleanup(shop_id: number, chainid: number | null) {
    ...
    const oldCustomerData = rawCustomerData.map((customer): CustomerObject => {
      ...
      customer.lastname != null
        ? (oldCustomer.lastname = customer.lastname.replace(/\s+/g, " ").trim())
        : "";
      customer.address1 != null
        ? (oldCustomer.address1 = customer.address1.replace(/\s+/g, " ").trim())
        : "";
      customer.address2 != null
        ? (oldCustomer.address2 = customer.address2.replace(/\s+/g, " ").trim())
        : "";
      customer.address_city != null
        ? (oldCustomer.address_city = customer.address_city
            .replace(/\s+/g, " ")
            .trim())
        : "";
      customer.address_state != null
        ? (oldCustomer.address_state = customer.address_state
            .replace(/\s+/g, " ")
            .trim())
        : "";
      customer.address_zip != null
        ? (oldCustomer.address_zip = customer.address_zip
            .replace(/\s+/g, " ")
            .trim())
        : "";
      oldCustomer.id = customer.id;
      oldCustomer.lastauthorized_date = customer.lastauthorized_date;
      oldCustomer.shopname = rawShopData[0].name;
      oldCustomer.shopemail = rawShopData[0].email;
      oldCustomer.shopphone = rawShopData[0].phone;
    ...
    const Customers = oldCustomerData.map(
      (
        ...
        let newNameCode: NameCodeObject = {
          firstname: "",
          lastname: "",
          resultname: "",
        };
